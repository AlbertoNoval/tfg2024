...
public class ShortestPath : MonoBehaviour
{
    ...
    public HashSet<Transform> findShortestPath(Transform start, Transform end)
    {

        nodes = GameObject.FindGameObjectsWithTag("casilla");
        ...
        List<Transform> lista = result.ToList<Transform>();
        lista.Reverse();
        HashSet<Transform> set = lista.ToHashSet<Transform>();
        return set;
    }
    ...
    private Transform DijkstrasAlgo(Transform start, Transform end)
    {
        ...
        while (unexplored.Count > 0)
        {
            ...
            HashSet<Transform> neighbours = currentNode.getNeighbourNode();
